...
# Paleta de colores (para uso en la interfaz, no en el PDF directamente)
MARCAS_COLORS = {
    "Einhell": (139, 50, 50),
    "KWB": (139, 50, 50),
    "Fijaciones": (204, 85, 0),
...
# ------------------------------------------------------------
# FUNCIONES DE OBTENCIÓN DE INFO DEL PRODUCTO (sin cambios)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# FUNCIÓN DE EXTRACCIÓN DE CATEGORÍAS (sin cambios)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# CONSTRUCCIÓN DE FILTROS (sin cambios)
# ------------------------------------------------------------
...
# ============================================================
# 4. RESUMEN DEL PEDIDO (con cálculos, sin cambios)
# ============================================================
...
if st.session_state.carrito:
    ...
    with col_btn2:
        if st.button("📄 Generar PDF del Pedido", type="primary", use_container_width=True):
            ...
            # ======================================================================
            # NUEVA GENERACIÓN DE PDF – ESTILO ERP MODERNO (REESCRITA COMPLETA)
            # ======================================================================
            ...
            MARGIN_LEFT = 15
            PAGE_WIDTH = 210 - 30  # 180 mm (aprovechamiento casi total)
            FONT_SIZE = 9          # Tamaño base para tabla
            FONT_SIZE_SMALL = 7    # Para descripción
            FONT_SIZE_TITLE = 18
            FONT_SIZE_TOTAL = 14
            ...
            def draw_title():
                ...
                pdf.cell(80, 12, sanitize_text("PROFORMA DE PEDIDO"), ln=True, align='R')
                pdf.ln(2)
            ...
            def draw_client_block():
                # Bloque de cliente a la izquierda (Bill To / Ship To)
                ...
                if codigo_cliente and pd.notna(codigo_cliente):
                    pdf.cell(0, 6, sanitize_text(f"{cliente_seleccionado} (Código: {codigo_cliente})"), ln=True)
                else:
                    pdf.cell(0, 6, sanitize_text(cliente_seleccionado), ln=True)
                pdf.cell(0, 6, sanitize_text(f"CUIT: {cli_info.get('C.U.I.T.', '-')}"), ln=True)
                ...
                if direccion_cliente and pd.notna(direccion_cliente):
                    pdf.cell(0, 6, sanitize_text(direccion_cliente), ln=True)
                pdf.cell(0, 6, sanitize_text(f"Condición: {cli_info.get('FORMA DE PAGO', '-')}"), ln=True)
                pdf.cell(0, 6, sanitize_text(f"Vendedor: {cli_info.get('NOMB.VENDEDOR', '-')}"), ln=True)
                if retira_local:
                    pdf.cell(0, 6, "Entrega: El cliente retira en el local", ln=True)
                elif direccion_entrega:
                    pdf.cell(0, 6, sanitize_text(f"Entrega: {direccion_entrega}"), ln=True)
                else:
                    pdf.cell(0, 6, "Entrega: En dirección del cliente (sin especificar)", ln=True)
            ...
            def draw_table_header(es_einhell):
                ...
                # Anchos redistribuidos (total 180 mm)
                if es_einhell:
                ...
                for i, h in enumerate(headers):
                    pdf.cell(widths[i], 8, h, border=0, align='C', fill=True)
                pdf.ln()
                pdf.set_text_color(0, 0, 0)
            ...
            def draw_product_row(row, es_einhell, widths):
                ...
                if es_einhell:
                    ...
                    pdf.set_font("Arial", '', FONT_SIZE)  # Resto en normal
                ...
                # Usamos MultiCell para que fluya, pero con altura máxima 2 líneas (aprox. 8 mm)
                # Como no queremos que ocupe demasiado, truncamos manualmente
                # Calculamos líneas
                ...
                if len(lines) > 2:
                    desc_display += "..."
                # Imprimimos la descripción truncada
                ...
                pdf.ln(4)  # Espacio después de la línea
            ...
            def draw_subtotal_block(marca, bruto, descuento, neto, iva, total):
                ...
                for lbl, val in zip(labels, values):
                    pdf.cell(36, 5, sanitize_text(f"{lbl}: {fmt_currency(val)}"), border=0, align='L')
                pdf.ln()
                pdf.ln(2)

            def draw_final_summary(total_bruto, total_descuento, total_neto, total_iva, total_final, texto_descuentos):
                # Bloque de totales alineado a la derecha (empaquetado)
                pdf.ln(6)
                pdf.set_font("Arial", 'B', 10)
                pdf.set_text_color(0, 0, 0)
                ...
                # Calculamos el ancho del bloque
                block_width = 80
                x_start = MARGIN_LEFT + PAGE_WIDTH - block_width
                pdf.set_x(x_start)

                # Usamos una tabla interna sin bordes para alinear a la derecha
                pdf.set_font("Arial", 'B', 9)
                pdf.cell(block_width, 6, "Subtotal Bruto:", border=0, align='L')
                ...
                pdf.set_x(x_start)
                pdf.set_font("Arial", 'B', 9)
                pdf.cell(block_width, 6, sanitize_text(f"Descuentos:"), border=0, align='L')
                pdf.set_x(x_start + 40)
                pdf.set_font("Arial", '', 9)
                ...
                pdf.set_x(x_start)
                pdf.set_font("Arial", 'B', 9)
                pdf.cell(block_width, 6, "Neto:", border=0, align='L')
                pdf.set_x(x_start + 40)
                pdf.set_font("Arial", '', 9)
                ...
                pdf.set_x(x_start)
                pdf.set_font("Arial", 'B', 9)
                pdf.cell(block_width, 6, "IVA Total:", border=0, align='L')
                pdf.set_x(x_start + 40)
                pdf.set_font("Arial", '', 9)
                ...
                pdf.cell(block_width - 4, 8, "TOTAL FINAL", border=0, align='L')
                pdf.set_x(x_start + 40)
            ...
            def draw_footer_notes():
                ...
                pdf.set_text_color(0, 0, 0)
                pdf.cell(0, 5, "Leyenda de colores por marca (solo en pantalla):", ln=True)
                pdf.set_font("Arial", '', 8)
